...
#ifndef RL_STATE_JSON_PATH
#define RL_STATE_JSON_PATH "/tmp/doomretro_state.json"
#endif

static void RL_WriteStateJSON(player_t *player)
{
    mobj_t *mo;
    FILE *file;
    ...
    file = fopen(RL_STATE_JSON_PATH ".tmp", "w");

    if (!file)
        return;

    fprintf(file, "{\n");
    fprintf(file, "  \"player_x\": %.3f,\n", (double)mo->x / FRACUNIT);
    fprintf(file, "  \"player_y\": %.3f,\n", (double)mo->y / FRACUNIT);
    fprintf(file, "  \"player_z\": %.3f,\n", (double)mo->z / FRACUNIT);
    fprintf(file, "  \"angle\": %u,\n", (unsigned int)mo->angle);
    fprintf(file, "  \"health\": %d,\n", player->health);
    fprintf(file, "  \"dead\": %s,\n", (player->playerstate == PST_DEAD || player->health <= 0) ? "true" : "false");
    fprintf(file, "  \"weapon\": %d\n", player->readyweapon);
    fprintf(file, "}\n");

    fclose(file);
    rename(RL_STATE_JSON_PATH ".tmp", RL_STATE_JSON_PATH);
}


void P_PlayerThink(void)
{
    RL_WriteStateJSON(viewplayer);
