...
def main():
    ...
    parser.add_argument("--advice-strict-route", action="store_true")
    parser.add_argument("--advice-align-deg", type=float, default=12.0)
    parser.add_argument("--advice-hard-stop-deg", type=float, default=110.0)
    parser.add_argument("--advice-disable-state-stuck-reset", action="store_true")
    parser.add_argument("--advice-combat-assist", action="store_true")
    parser.add_argument("--advice-combat-health", type=int, default=70)
    parser.add_argument("--advice-combat-hold", type=float, default=1.25)
    parser.add_argument("--advice-suppress-shoot-unless-combat", action="store_true")
        ...
        with mss.mss() as sct:
            ...
            while True:
                ...
                if advice:
                    ...
                    combat = advice.get("combat", {})
                    survival = advice.get("survival", {})
                    ammo_info = advice.get("ammo", {})

                    combat_status = combat.get("status", "quiet")
                    health_now = int(survival.get("health", 999))
                    ammo_now = int(ammo_info.get("total", 0))

                    combat_now = (
                        args.advice_combat_assist
                        and ammo_now > 0
                        and (
                            combat_status == "taking_damage"
                            or health_now <= args.advice_combat_health
                        )
                    )

                    if combat_now:
                        combat_until = time.time() + args.advice_combat_hold

                    combat_active = (
                        args.advice_combat_assist
                        and ammo_now > 0
                        and time.time() < combat_until
                    )
                    ...
                    if (
                        args.advice_no_shoot_offroute
                        and route_status == "OFF_ROUTE"
                        and not combat_active
                    ):
                        shoot_prob = 0.0

                    if args.advice_strict_route:
                        # Strong map-progress mode:
                        # If route progress says recover/turn, ignore BC/model movement.
                        # Rotate first. Only move forward when aligned enough.
                        if route_advice == "finish_or_use":
                            wanted_keys = set()
                            if args.advice_final_use:
                                tap_use = True
                        ...
                        elif route_status == "OFF_ROUTE" or abs(turn_error) >= args.advice_stop_turn_deg:
                            wanted_keys = set()
                            dx = turn_dx

                            # Strong route recovery, but do not freeze in place
                            # unless the heading error is extremely large.
                            if abs(turn_error) < args.advice_hard_stop_deg:
                                wanted_keys.add("w")

                            if not combat_active:
                                shoot_prob = 0.0

                        elif route_advice in ("turn_left", "turn_right"):
                            wanted_keys = set()
                            dx = turn_dx

                            # Move while steering. This prevents stop-spin-stop-spin.
                            if abs(turn_error) < args.advice_hard_stop_deg:
                                wanted_keys.add("w")

                        elif route_advice == "move_forward":
                            wanted_keys = set()
                            wanted_keys.add("w")

                if args.advice_suppress_shoot_unless_combat and not combat_active:
                    shoot_prob = 0.0

                if args.advice_suppress_shoot_unless_combat and not combat_active:
                    shoot_prob = 0.0

                if combat_active and route_advice != "finish_or_use":
                    shoot_prob = max(shoot_prob, args.shoot_threshold + 0.10)
                ...
                # In strict route mode, the watcher/advice layer handles recovery.
                # The old F9 stuck reset can fight route recovery, so allow disabling it.
                pos = None if args.advice_disable_state_stuck_reset else read_player_position(args.state_json)
                if pos is not None:
                    if last_pos is None:
                        last_pos = pos
