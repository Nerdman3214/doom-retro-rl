...
class RewardManager:
    ...
    def detect_grid_exploration(self, game_state):
        ...
        grid_cell = (int(x // 32), int(y // 32))
        if grid_cell not in self.visited_cells:
            self.visited_cells.add(grid_cell)
            self.add("grid_exploration", +0.35)
        else:
            self.add("exploration_consistency", +0.01)

    def detect_health_and_ammo_ratio(self, game_state):
        health_ratio = max(0.0, min(game_state.get("health", 0) / 100.0, 1.0))
        ammo_ratio = max(0.0, min(game_state.get("ammo", 0) / 100.0, 1.0))
        self.add("health_ratio", 0.12 * health_ratio)
        self.add("ammo_ratio", 0.08 * ammo_ratio)

    def detect_movement_progress(self, game_state):
        x = game_state.get("x")
        y = game_state.get("y")
        if x is None or y is None:
            return
        ...
        if self.previous_position is not None:
            ...
            if distance > 8.0:
                progress = min(distance / 80.0, 1.0)
