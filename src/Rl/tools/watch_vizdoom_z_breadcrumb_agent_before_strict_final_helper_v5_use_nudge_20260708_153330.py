...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper v4:
            # Use the real route/waypoint distance near wp260, not the terminal_d2 helper value.
            # V3 could think d2 was tiny while target_d2 was still ~20-30, causing USE spam.
            ...
            if final_button_mode:
                try:
                    final_d2_now = float(target_d2)
                except Exception:
                    final_d2_now = float(locals().get("terminal_d2", 999.0))

                # Prefer the route heading error if available. Fall back to terminal_err.
                try:
                    final_err_now = float(locals().get("target_err", locals().get("err", locals().get("terminal_err", 0.0))))
                except Exception:
                    final_err_now = 0.0

                # More conservative than v3:
                # - If far from the final waypoint/button area, walk forward.
                # - If close but off-angle, turn.
                # - Only press use when both close and aligned.
                approach_until_d2 = 12.0
                press_face_deg = 16.0

                buttons = [0] * len(buttons)

                if final_d2_now > approach_until_d2:
                    if abs(final_err_now) > 24.0:
                        if final_err_now > 0 and len(buttons) >= 3:
                            buttons[2] = 1
                            source = (
                                "strict_final_route_align_left:"
                                + f"route_d2={final_d2_now:.1f}:err={final_err_now:.1f}:"
                                + str(source)
                            )
                        elif final_err_now < 0 and len(buttons) >= 4:
                            buttons[3] = 1
                            source = (
                                "strict_final_route_align_right:"
                                + f"route_d2={final_d2_now:.1f}:err={final_err_now:.1f}:"
                                + str(source)
                            )
                    else:
                        if len(buttons) >= 1:
                            buttons[0] = 1
                        source = (
                            "strict_final_route_approach:"
                            + f"route_d2={final_d2_now:.1f}:err={final_err_now:.1f}:"
                            + str(source)
                        )

                elif abs(final_err_now) > press_face_deg:
                    if final_err_now > 0 and len(buttons) >= 3:
                        buttons[2] = 1
                        source = (
                            "strict_final_close_align_left:"
                            + f"route_d2={final_d2_now:.1f}:err={final_err_now:.1f}:"
                            + str(source)
                        )
                    elif final_err_now < 0 and len(buttons) >= 4:
                        buttons[3] = 1
                        source = (
                            "strict_final_close_align_right:"
                            + f"route_d2={final_d2_now:.1f}:err={final_err_now:.1f}:"
                            + str(source)
                        )
