...
# Rail scrape breaker:
# If the agent keeps holding one strafe direction, it is probably scraping a rail/wall.
RAIL_STRAFE_LOCK_LIMIT = 10
RAIL_RECOVERY_FRAMES = 12
...
def main():
    ...
    rail_recovery_frames = 0
    rail_recovery_side = 0
    strafe_right_lock_frames = 0
    strafe_left_lock_frames = 0
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            # RAIL_SCRAPE_BREAKER
            # Detect repeated one-direction strafe lock.
            if "strafe_right" in active and "strafe_left" not in active:
                strafe_right_lock_frames += 1
            else:
                strafe_right_lock_frames = 0

            if "strafe_left" in active and "strafe_right" not in active:
                strafe_left_lock_frames += 1
            else:
                strafe_left_lock_frames = 0

            if strafe_right_lock_frames >= RAIL_STRAFE_LOCK_LIMIT:
                rail_recovery_frames = RAIL_RECOVERY_FRAMES
                rail_recovery_side = -1
                strafe_right_lock_frames = 0

            if strafe_left_lock_frames >= RAIL_STRAFE_LOCK_LIMIT:
                rail_recovery_frames = RAIL_RECOVERY_FRAMES
                rail_recovery_side = 1
                strafe_left_lock_frames = 0

            if rail_recovery_frames > 0:
                active.discard("mouse_left")
                active.discard("use")
                active.discard("move_backward")
                active.discard("strafe_left")
                ...
                if rail_recovery_side < 0:
                    # Was stuck scraping right, escape left.
                    active.add("strafe_left")
                    dx = -0.5
                elif rail_recovery_side > 0:
                    # Was stuck scraping left, escape right.
                    active.add("strafe_right")
                    dx = 0.5

                dy = 0.0
                rail_recovery_frames -= 1
