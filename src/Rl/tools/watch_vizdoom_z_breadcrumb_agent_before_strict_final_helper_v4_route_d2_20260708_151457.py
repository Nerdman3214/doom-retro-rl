...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper v3:
            # Do NOT spam USE just because we are close.
            # First face the final button, then approach, then press USE.
            ...
            if final_button_mode:
                try:
                    terminal_d2_now = float(locals().get("terminal_d2", target_d2))
                except Exception:
                    terminal_d2_now = float(target_d2)

                try:
                    terminal_err_now = float(locals().get("terminal_err", 0.0))
                except Exception:
                    terminal_err_now = 0.0

                # Keep this strict. The previous helper could press USE while facing far off-angle.
                press_radius = 24.0
                press_face_deg = 20.0
                align_face_deg = 14.0
                ...
                if abs(terminal_err_now) > press_face_deg:
                    if terminal_err_now > 0 and len(buttons) >= 3:
                        buttons[2] = 1
                        source = (
                            "strict_final_align_left:"
                            + f"d2={terminal_d2_now:.1f}:err={terminal_err_now:.1f}:"
                            + str(source)
                        )
                    elif terminal_err_now < 0 and len(buttons) >= 4:
                        buttons[3] = 1
                        source = (
                            "strict_final_align_right:"
                            + f"d2={terminal_d2_now:.1f}:err={terminal_err_now:.1f}:"
                            + str(source)
                        )

                elif terminal_d2_now > press_radius:
                    if len(buttons) >= 1:
                        buttons[0] = 1
                    source = (
                        "strict_final_approach:"
                        + f"d2={terminal_d2_now:.1f}:err={terminal_err_now:.1f}:"
                        + str(source)
                    )

                elif abs(terminal_err_now) <= press_face_deg:
                    if len(buttons) >= 8:
                        buttons[7] = 1
                    source = (
                        "strict_final_button_use:"
                        + f"d2={terminal_d2_now:.1f}:err={terminal_err_now:.1f}:"
                        + str(source)
                    )

                else:
                    # Safety fallback: if uncertain, align instead of wasting USE presses.
                    if terminal_err_now >= 0 and len(buttons) >= 3:
                        buttons[2] = 1
                        source = "strict_final_align_left_fallback:" + str(source)
                    elif len(buttons) >= 4:
                        buttons[3] = 1
                        source = "strict_final_align_right_fallback:" + str(source)
