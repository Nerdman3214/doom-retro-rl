...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper v5:
            # Final behavior should be simple and stable:
            # face button -> press USE -> if not complete, nudge forward -> face button -> USE again.
            # Do not use route-error turning here because it can make the agent look away from the button.
            ...
            if final_button_mode:
                try:
                    final_terminal_d2 = float(locals().get("terminal_d2", target_d2))
                except Exception:
                    final_terminal_d2 = float(target_d2)
                ...
                # Tight enough to avoid looking away, loose enough to press use without endless turning.
                face_deg = 13.0
                approach_radius = 28.0
                nudge_steps_after_use = 5

                buttons = [0] * len(buttons)

                if abs(final_button_err) > face_deg:
                    final_use_nudge_countdown = 0
                    if final_button_err > 0 and len(buttons) >= 3:
                        buttons[2] = 1
                        source = (
                            "strict_final_face_left_v5:"
                            + f"td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                            + str(source)
                        )
                    elif final_button_err < 0 and len(buttons) >= 4:
                        buttons[3] = 1
                        source = (
                            "strict_final_face_right_v5:"
                            + f"td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                            + str(source)
                        )

                elif final_use_nudge_countdown > 0:
                    # We already tried USE and it did not end the episode.
                    # Move forward a few ticks, but only while still facing the button.
                    if len(buttons) >= 1:
                        buttons[0] = 1
                    final_use_nudge_countdown -= 1
                    source = (
                        "strict_final_nudge_forward_v5:"
                        + f"remaining={final_use_nudge_countdown}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                        + str(source)
                    )

                elif final_terminal_d2 > approach_radius:
                    # Too far for final use. Walk toward button while aligned.
                    if len(buttons) >= 1:
                        buttons[0] = 1
                    source = (
                        "strict_final_approach_button_v5:"
                        + f"td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                        + str(source)
                    )

                else:
                    # Try the real final button. If this does not finish the episode,
                    # the next ticks will nudge forward and try again.
                    if len(buttons) >= 8:
                        buttons[7] = 1
                    final_use_attempt_count += 1
                    final_use_nudge_countdown = nudge_steps_after_use
                    source = (
                        "strict_final_use_then_nudge_v5:"
                        + f"attempt={final_use_attempt_count}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                        + str(source)
                    )
