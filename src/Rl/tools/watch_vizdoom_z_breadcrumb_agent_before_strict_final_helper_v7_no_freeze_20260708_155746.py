...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper v6:
            # Small-turn version.
            # Important idea:
            # - far/medium: face button using 1-tick turn pulses
            # - close: stop turning so we do not look away
            # - use, then make a tiny forward nudge only if still not complete
            final_button_mode = any(tag in str(source) for tag in (
                "terminal_magnet_use",
                "terminal_use_lock",
                "terminal_use_lock_start",
                "terminal_magnet_final_face",
            ...
            if final_button_mode:
                try:
                    ...
                    final_route_d2 = float(locals().get("terminal_d2", 999.0))
                ...
                try:
                    final_button_err = float(locals().get("terminal_err", 0.0))
                except Exception:
                    final_button_err = 0.0

                buttons = [0] * len(buttons)

                # Decay turn cooldown. This creates "small turns":
                # one tick of turn, then several ticks of waiting/approach.
                ...
                # Very close to the recorded final point:
                # do NOT keep turning from unstable terminal_err. Press use and occasionally nudge.
                if final_route_d2 <= close_route_d2:
                    if final_use_nudge_countdown > 0 and final_route_d2 > 2.0:
                        if len(buttons) >= 1:
                            buttons[0] = 1
                        final_use_nudge_countdown -= 1
                        source = (
                            "strict_final_small_nudge_v6:"
                            + f"remaining={final_use_nudge_countdown}:route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                            + str(source)
                        )
                    else:
                        if len(buttons) >= 8:
                        ...
                        source = (
                            "strict_final_use_close_v6:"
                            + f"attempt={final_use_attempt_count}:route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                            + str(source)
                        )

                # Not close yet: use small turn pulses, not continuous turn spam.
                elif abs(final_button_err) > face_deg_far:
                    if final_turn_pulse_cooldown == 0:
                        if final_button_err > 0 and len(buttons) >= 3:
                            buttons[2] = 1
                            final_turn_pulse_cooldown = turn_pulse_gap
                            source = (
                                "strict_final_small_turn_left_v6:"
                                + f"cooldown={final_turn_pulse_cooldown}:route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                                + str(source)
                            )
                        elif final_button_err < 0 and len(buttons) >= 4:
                            buttons[3] = 1
                            final_turn_pulse_cooldown = turn_pulse_gap
                            source = (
                                "strict_final_small_turn_right_v6:"
                                + f"cooldown={final_turn_pulse_cooldown}:route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                                + str(source)
                            )
                    else:
                        source = (
                            "strict_final_turn_wait_v6:"
                            + f"cooldown={final_turn_pulse_cooldown}:route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                            + str(source)
                        )

                # Facing close enough but not at the button yet: move forward.
                else:
                    if len(buttons) >= 1:
                        buttons[0] = 1
                    source = (
                        "strict_final_approach_aligned_v6:"
                        + f"route_d2={final_route_d2:.1f}:td2={final_terminal_d2:.1f}:terr={final_button_err:.1f}:"
                        + str(source)
                    )
    ...
    if ai_record_dir is not None:
        ...
        ai_success = (
            ai_record_max_wp >= min_success_wp
            and final_health > 0
            and ai_record_true_episode_finished
            and ai_record_final_action_used
        )

        if ai_record_rows:
            ai_record_rows[-1]["done"] = True

        meta = {
            "success": bool(ai_success),
            "max_wp": int(ai_record_max_wp),
            "total_wp": int(total_wp),
            "min_success_wp": int(min_success_wp),
            "final_health": final_health,
            "terminal_signal": bool(ai_record_terminal_signal),
            "true_episode_finished": bool(ai_record_true_episode_finished),
            "final_action_used": bool(ai_record_final_action_used),
            "final_action_name": str(ai_record_final_action_name),
            "strict_success_rule": "success requires game.is_episode_finished() after a use action",
            "rows": len(ai_record_rows),
            "duration_seconds": time.time() - ai_record_start_time,
            "route_session": str(args.route_session),
            "iwad": str(args.iwad),
            "map": str(args.map),
        ...
        if ai_success:
            ...
            print(f"[ai_record] SUCCESS saved: {final_dir}")
        else:
            print(
