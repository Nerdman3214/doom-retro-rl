...
def simplify_active(probs: torch.Tensor) -> tuple[set[str], dict[str, float]]:
    ...
    # IMITATION_COMBAT_CLEANUP
    # The model tends to shoot while standing still.
    # In Doom, useful combat usually still keeps some forward pressure.
    if "mouse_left" in active and "move_forward" not in active:
        if prob_by_name.get("move_forward", 0.0) >= 0.15:
            active.add("move_forward")
        else:
            # If it only wants to shoot and has no movement intent, do not let it turret forever.
