...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper:
            # Near the terminal, only a real USE button press should be allowed to finish/save success.
            # Button order in this project is:
            # move_forward, move_backward, turn_left, turn_right, strafe_left, strafe_right, shoot, use
            if any(tag in str(source) for tag in (
                "terminal_magnet_use",
                "terminal_use_lock",
                "terminal_use_lock_start",
            )):
                ...
                if len(buttons) >= 8:
                    buttons[7] = 1
                source = "strict_final_button_use:" + str(source)
