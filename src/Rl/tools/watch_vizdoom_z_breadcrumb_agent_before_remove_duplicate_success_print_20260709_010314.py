...
def main():
    ...
    if ai_record_dir is not None:
        ...
        if ai_success:
            ...
            # FALLBACK SUCCESS META FOR TERMINAL SIGNAL:
            # The forced terminal-signal success path may reach this write before
            # the original code creates meta, so create a safe metadata object here.
            try:
                meta
            except NameError:
                meta = {
                    "success": True,
                    "forced_terminal_signal_success": bool(locals().get("ai_record_terminal_signal_forced_success", False)),
                    ...
                    "true_episode_finished": bool(locals().get("ai_record_true_episode_finished", False)),
                    "final_action_used": bool(locals().get("ai_record_final_action_used", False)),
                    "final_action_name": str(locals().get("ai_record_final_action_name", "")),
                    "strict_success_rule": "success saved when terminal_signal=True and final_health > 0",
                }
            print(f"[ai_record] SUCCESS saved (TERMINAL SIGNAL): {ai_record_dir} "
                  f"terminal_signal={ai_record_terminal_signal} "
                  f"forced={locals().get('ai_record_terminal_signal_forced_success', False)} "
