...
def main():
    ...
    p.add_argument("--turn-deadzone", type=float, default=20.0)
    p.add_argument("--turn-release", type=float, default=10.0)
    p.add_argument("--turn-smoothing", type=float, default=0.85)
    p.add_argument("--recovery-distance", type=float, default=220.0)
    p.add_argument("--recovery-angle-limit", type=float, default=70.0)
    ...
    p.add_argument("--route-reset-distance", type=float, default=96.0)
    p.add_argument("--route-reset-after-wp", type=int, default=15)
    ...
    try:
        while True:
            ...
            # If the game was reset/reloaded and the player is back near the
            # route start, reset progress-lock too. Otherwise the watcher can
            # think the "correct" waypoint is later in the map and cause a 180.
            ...
            # Corridor heading:
            # Usually follow the route tangent instead of aiming directly at
            # a waypoint point. Point-pursuit can cause 180 turns when the
            # player is beside or slightly past the target waypoint.
            route_base = route[i]
            ...
            # If we are not extremely far from the route, avoid huge 180-style
            # corrections. Keep progress forward and gently bend back instead.
            ...
            # Smooth route error so tiny target/waypoint changes do not flip
            # left/right every frame.
            ...
            # Hysteresis: once turning left/right, keep that direction until
            # the error is safely inside the release zone.
            # Near 180 degrees, tiny noise can flip +179 to -179.
            # Keep the previous turn direction instead of oscillating.
            if abs(turn_error) >= 160.0 and stable_route_advice in ("turn_left", "turn_right"):
                stable_route_advice = stable_route_advice
