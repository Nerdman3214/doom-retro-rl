...
for d in sessions:
    ...
    # Important: no_bc is only informational because actions.csv may not preserve
    # bc_policy sources even when terminal logs prove BC controlled the run.
    if bc == 0:
        notes.append("no_bc_seen_in_actions_csv")

    # final_wall around 40-60 is normal for current scripted rescue.
    if final_wall > 120:
...
report = out_dir / "bc_mix010_quality_report_v2.csv"
...
print("[bc_quality_v2] sessions checked:", len(rows_out))
print("[bc_quality_v2] clean:", len(clean))
print("[bc_quality_v2] messy:", len(messy))
print("[bc_quality_v2] report:", report)

print()
print("[bc_quality_v2] clean sessions:")
for r in clean[:80]:
    print(
        "CLEAN",
        r["session"],
...
print()
print("[bc_quality_v2] messy sessions:")
for r in messy[:80]:
    print(
        "MESSY",
        r["session"],
