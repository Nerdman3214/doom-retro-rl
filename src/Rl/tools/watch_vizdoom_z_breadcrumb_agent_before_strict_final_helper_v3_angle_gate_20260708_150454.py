...
def main():
    ...
    try:
        while not game.is_episode_finished():
            ...
            # Strict final-button helper v2:
            # The level should only complete from a real USE action, but do not spam USE early.
            # First align to the terminal, then approach, then press USE.
            ...
            if final_button_mode:
                ...
                use_radius = float(getattr(args, "terminal_magnet_use_radius", 18.0))
                face_deg = float(getattr(args, "terminal_use_lock_face_deg", 20.0))
                face_deg = max(12.0, face_deg)

                buttons = [0] * len(buttons)

                if abs(terminal_err_now) > face_deg:
                    # Same turn convention used by the route controller:
                    # positive error -> turn_left, negative error -> turn_right.
                ...
                elif terminal_d2_now > use_radius:
                    if len(buttons) >= 1:
                        buttons[0] = 1
                    source = (
