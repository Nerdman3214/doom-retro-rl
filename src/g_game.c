...
//
#if !defined(_WIN32)
// RL shared memory bridge — writes player state every game tic so that the
// Python RL agent can read real health/ammo/position without screen OCR.
// Layout must match shm_reader.py: health, ammo[4], x, y, angle, armor, kills.
#pragma pack(push, 1)
typedef struct
{
    int32_t     health;
    int32_t     ammo[4];    // clip, shell, cell, misl
    int32_t     x;          // fixed_t (divide by 65536 for map units)
    int32_t     y;
    uint32_t    angle;      // angle_t (0xFFFFFFFF == 360 degrees)
    int32_t     armor;
    int32_t     kills;
} rl_state_t;
#pragma pack(pop)

static int          rl_shm_fd   = -1;
static rl_state_t  *rl_state    = NULL;

static void RL_WriteState(void)
{
    if (!viewplayer || !viewplayer->mo)
        return;

    // Open/map the shared memory segment on first call.
    if (rl_shm_fd < 0)
    {
        rl_shm_fd = shm_open("/doomretro_rl", O_CREAT | O_RDWR, 0666);
        if (rl_shm_fd < 0)
            return;
        if (ftruncate(rl_shm_fd, sizeof(rl_state_t)) < 0)
            return;
        rl_state = mmap(NULL, sizeof(rl_state_t),
                        PROT_READ | PROT_WRITE, MAP_SHARED,
                        rl_shm_fd, 0);
        if (rl_state == MAP_FAILED)
        {
            rl_state = NULL;
            return;
        }
    ...
    rl_state->health    = viewplayer->mo->health;
    rl_state->ammo[0]   = viewplayer->ammo[am_clip];
    rl_state->ammo[1]   = viewplayer->ammo[am_shell];
    rl_state->ammo[2]   = viewplayer->ammo[am_cell];
    rl_state->ammo[3]   = viewplayer->ammo[am_misl];
    rl_state->x         = viewplayer->mo->x;
    rl_state->y         = viewplayer->mo->y;
    rl_state->angle     = viewplayer->mo->angle;
    rl_state->armor     = viewplayer->armor;
    rl_state->kills     = viewplayer->killcount;
}
#endif // !_WIN32
